...
<h2>D435I localization tuner (no actuation)</h2><p>Drag a box around the plushie, enter its torso-frame XYZ in metres, then save.</p>
<img id=f src=/frame.jpg><div><label>name <input id=n value=pose1></label><label>X <input id=x type=number step=.001></label><label>Y <input id=y type=number step=.001></label><label>Z <input id=z type=number step=.001></label><button onclick=save()>Save sample</button></div><div id=msg></div>
<script>let a,b,box;f.onpointerdown=e=>{let r=f.getBoundingClientRect();a=[(e.clientX-r.left)*960/r.width,(e.clientY-r.top)*540/r.height];box=null};f.onpointerup=e=>{let r=f.getBoundingClientRect();b=[(e.clientX-r.left)*960/r.width,(e.clientY-r.top)*540/r.height];box=[Math.min(a[0],b[0]),Math.min(a[1],b[1]),Math.max(a[0],b[0]),Math.max(a[1],b[1])];msg.textContent='box '+box.map(v=>v.toFixed(1)).join(', ')};setInterval(()=>f.src='/frame.jpg?t='+Date.now(),250);async function save(){if(!box)return msg.textContent='draw a box first';let r=await fetch('/sample',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:n.value,bbox_xyxy:box,torso_truth_m:[+x.value,+y.value,+z.value]})});msg.textContent=await r.text()}</script>"""
