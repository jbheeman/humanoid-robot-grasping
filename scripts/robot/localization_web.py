...
<h2>D435I localization tuner (no actuation)</h2><p>Place the bunny at the marked tabletop position. Drag a box around it, then enter its position relative to the robot torso: X=robot-left/right, Y=robot-forward, Z=height above the floor, all in metres.</p>
<img id=f draggable=false src=/frame.jpg><div><label>name <input id=n value=pose1></label><label>torso X <input id=x type=number step=.001 placeholder="left/right"></label><label>torso Y <input id=y type=number step=.001 placeholder="forward"></label><label>torso Z <input id=z type=number step=.001 placeholder="height"></label><button onclick=save()>Save sample</button></div><div id=msg></div>
