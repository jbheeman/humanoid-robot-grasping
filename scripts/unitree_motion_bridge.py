...
G1_LOCO_SCRIPT = REPO_ROOT / "scripts" / "g1_loco.py"
...
def make_handler(bridge: Bridge) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if path == "/move":
                if not bridge.args.allow_movement:
                    self.send_json(403, {"ok": False, "error": "Start bridge with --allow-movement to use /move."})
                    return
                try:
