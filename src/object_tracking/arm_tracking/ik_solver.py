...
def adaptive_table_route(
    ...
    *,
    ...
    maximum_segment_m: float = 0.025,
) -> CartesianTableRoute:
    """Build a variable-length route around an inflated tabletop footprint."""

    start = np.asarray(start_xyz_m, dtype=float)
    goal = np.asarray(goal_xyz_m, dtype=float)
    if (
        start.shape != (3,)
        or goal.shape != (3,)
        or not np.all(np.isfinite(np.concatenate((start, goal))))
        or side_margin_m < 0.0
        or top_clearance_m <= 0.0
        or maximum_segment_m <= 0.0
    ):
        raise ValueError("invalid adaptive table route inputs")
    if not all(
        hasattr(support_region, name)
        for name in (
            "projection_uv",
            "origin",
            "axis_u",
        ...
        raise ValueError("adaptive route requires a bounded support region")

    def from_uvh(uv: np.ndarray, height: float) -> np.ndarray:
        return (
            support_region.origin
            + uv[0] * support_region.axis_u
            + uv[1] * support_region.axis_v
            + height * support_region.plane.normal
        )

    start_uv = support_region.projection_uv(start)
    goal_uv = support_region.projection_uv(goal)
    start_height = float(support_region.signed_distance(start))
    goal_height = max(
        float(support_region.signed_distance(goal)),
        top_clearance_m,
    )
    topology = support_region.classify_point(
        start,
        side_margin_m=side_margin_m,
        top_clearance_m=top_clearance_m,
    )
    inflated_min = np.asarray(support_region.minimum_uv) - side_margin_m
    inflated_max = np.asarray(support_region.maximum_uv) + side_margin_m
    semantic: list[np.ndarray] = [start]
    exit_edge: str | None = None

    if topology == "under_or_inside":
        exit_candidates: list[tuple[float, str, np.ndarray]] = []
        for edge in support_region.certified_edges:
            exit_uv = start_uv.copy()
            if edge == "u_min":
                exit_uv[0] = inflated_min[0] - 0.01
            elif edge == "u_max":
                exit_uv[0] = inflated_max[0] + 0.01
            elif edge == "v_min":
        ...
        semantic.append(from_uvh(exit_uv, start_height))
        semantic.append(from_uvh(exit_uv, top_clearance_m))
    elif topology == "outside":
        semantic.append(from_uvh(start_uv, top_clearance_m))

    semantic.append(from_uvh(goal_uv, goal_height))

    compact: list[np.ndarray] = []
    for point in semantic:
        if not compact or float(np.linalg.norm(point - compact[-1])) > 1e-7:
            compact.append(point)
    dense: list[np.ndarray] = [compact[0]]
...
class G1RightArmIK:
    ...
    def plan_adaptive_table_approach(
        self,
        ...
        *,
        ...
        if escape_path is None:
            return IKPathResult(
                ...
                float("inf"),
                0.0,
                escape_error or "adaptive_start_escape_unavailable",
            )
        path = list(escape_path)
        q = np.asarray(path[-1], dtype=float)
        start_transform = self.forward_kinematics(q)
        try:
            cartesian = adaptive_table_route(
                start_transform[:3, 3],
                target[:3, 3],
                support_plane,
                side_margin_m=side_margin_m,
                top_clearance_m=top_clearance_m,
                maximum_segment_m=maximum_cartesian_segment_m,
            )
    ...
    def solve_with_collision_detour(
        ...
        *,
        ...
        if not enforce_orientation and hasattr(support_plane, "classify_point"):
            adaptive = self.plan_adaptive_table_approach(
                target,
                start_q,
                support_plane=support_plane,
            )
